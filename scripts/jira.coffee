...
#   hubot show jira watchers for <Issue Key> - Shows watchers for the given JIRA issue
#   hubot search jira for <JQL> - Search JIRA with JQL
#   hubot save jira filter <JQL> as <name> - Save JIRA JQL query as filter in the brain
#   hubot use jira filter <name> - Use a JIRA filter from the brain
#   hubot show jira filter(s) - Show all JIRA filters
#   hubot show jira filter <name> - Show a specific JIRA filter
#
# Author:
#   codec
...
module.exports = (robot) ->
  ...
  robot.respond /(show )? jira watchers (for )?(\w+-[0-9]+)/i, (msg) ->
  ...
  robot.respond /search jira (for )?(.*)/i, (msg) ->
    if msg.message.user.id is robot.name
      return
  ...
  robot.respond /save jira filter (.*) as (.*)/i, (msg) ->
  ...
  robot.respond /delete jira filter (.*)/i, (msg) ->
    filters.delete msg.match[1]

  robot.respond /(use )?jira filter (.*)/i, (msg) ->
    name    = msg.match[2]
  ...
  robot.respond /(show )?jira filter(s)? ?(.*)?/i, (msg) ->
